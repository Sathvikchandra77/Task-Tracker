  ...
  stages {
    ...
    stage('Deploy (Staging)') {
      steps {
        ...
        // wait 3 seconds in a way Jenkins supports
        bat 'powershell -NoProfile -NonInteractive -Command "Start-Sleep -Seconds 3"'
        bat 'powershell -NoProfile -NonInteractive -Command "Invoke-WebRequest http://localhost:8081/health -UseBasicParsing | Out-Null"'
      }
    }
    ...
    stage('Release (Prod)') {
      ...
      steps {
        ...
        bat 'powershell -NoProfile -NonInteractive -Command "Start-Sleep -Seconds 5"'
        bat 'powershell -NoProfile -NonInteractive -Command "Invoke-WebRequest http://localhost:8080/health -UseBasicParsing | Out-Null"'
      }
    }

    stage('Monitoring & Evidence') {
      steps {
        bat 'powershell -NoProfile -NonInteractive -Command "Invoke-WebRequest http://localhost:8080/health -UseBasicParsing | Out-File -Encoding utf8 health_status.json"'
